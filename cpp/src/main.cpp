...
// Target: RDK X5, Bayes-e. Port of the MIT S100P reference demo.
//
// X5 hbDNNInfer asynchronous tasks, one worker per model. CPU preprocessing,
// BPU execution and rendering overlap. Independent latest-frame queues expose
// actual model throughput and sequence age; display never counts duplicate camera frames.
//
...
// ARM NEON bilinear scaling. Keep BGR byte order via libyuv RGB24 (B,G,R).
// This avoids the slow generic OpenCV resize path on the installed BSP.
static void FastResize(const cv::Mat& src, cv::Mat& dst, cv::Size size,
                       double fx=0, double fy=0, int interpolation=cv::INTER_LINEAR) {
  if(src.size()==size) {src.copyTo(dst); return;}
  if(src.type()!=CV_8UC3) {cv::resize(src,dst,size,fx,fy,interpolation); return;}
  cv::Mat argb(src.rows,src.cols,CV_8UC4), scaled(size,CV_8UC4);
  libyuv::RGB24ToARGB(src.data,src.step,argb.data,argb.step,src.cols,src.rows);
  libyuv::ARGBScale(argb.data,argb.step,src.cols,src.rows,scaled.data,scaled.step,
                   size.width,size.height,libyuv::kFilterBilinear);
  dst.create(size,CV_8UC3);
  libyuv::ARGBToRGB24(scaled.data,scaled.step,dst.data,dst.step,size.width,size.height);
}
...
// X5 Bayes-e runtime: packed NV12 and hbSysMem[4], no S100 UCP API.
class DnnModel {
 public:
  DnnModel(hbPackedDNNHandle_t packed, const std::string& name) : name_(name) {
    HB_CHECK(hbDNNGetModelHandle(&handle_, packed, name.c_str()), "model");
    int ni=0, no=0;
    HB_CHECK(hbDNNGetInputCount(&ni, handle_), "inputs");
    HB_CHECK(hbDNNGetOutputCount(&no, handle_), "outputs");
    if (ni != 1) throw std::runtime_error("X5 requires one packed NV12 input");
    inputs_.resize(ni); outputs_.resize(no);
    for (int i=0;i<ni;++i) {
      auto& t=inputs_[i];
      HB_CHECK(hbDNNGetInputTensorProperties(&t.properties,handle_,i), "input properties");
      if(t.properties.tensorType != HB_DNN_IMG_TYPE_NV12)
        throw std::runtime_error("model must use packed NV12");
      HB_CHECK(hbSysAllocCachedMem(&t.sysMem[0],t.properties.alignedByteSize),"input memory");
    }
    for(int i=0;i<no;++i) {
      auto& t=outputs_[i];
      HB_CHECK(hbDNNGetOutputTensorProperties(&t.properties,handle_,i),"output properties");
      auto& p=t.properties;
      if(p.tensorType != HB_DNN_TENSOR_TYPE_F32 || p.quantiType != NONE)
        throw std::runtime_error("requires official dequantized float32 outputs");
      for(int k=0;k<p.validShape.numDimensions;++k)
        if(p.validShape.dimensionSize[k] != p.alignedShape.dimensionSize[k])
          throw std::runtime_error("output padding requires explicit unpacking");
      HB_CHECK(hbSysAllocCachedMem(&t.sysMem[0],p.alignedByteSize),"output memory");
      std::printf("[tensor] %s output %d bytes=%d shape=",name.c_str(),i,p.alignedByteSize);
      for(int k=0;k<p.validShape.numDimensions;++k) std::printf("%d,",p.validShape.dimensionSize[k]);
      std::puts("");
    }
  }
  ~DnnModel() {
    for(auto& t:inputs_) if(t.sysMem[0].virAddr) hbSysFreeMem(&t.sysMem[0]);
    for(auto& t:outputs_) if(t.sysMem[0].virAddr) hbSysFreeMem(&t.sysMem[0]);
  }
  double Run(int priority, uint64_t) {
    HB_CHECK(hbSysFlushMem(&inputs_[0].sysMem[0],HB_SYS_MEM_CACHE_CLEAN),"clean");
    hbDNNInferCtrlParam ctl;
    HB_DNN_INITIALIZE_INFER_CTRL_PARAM(&ctl);
    ctl.priority=priority;
    hbDNNTaskHandle_t task=nullptr;
    hbDNNTensor* out=outputs_.data();
    auto t0=NOW();
    HB_CHECK(hbDNNInfer(&task,&out,inputs_.data(),handle_,&ctl),"infer");
    HB_CHECK(hbDNNWaitTaskDone(task,5000),"wait");
    double ms=ms_since(t0);
    HB_CHECK(hbDNNReleaseTask(task),"release task");
    for(auto& t:outputs_) HB_CHECK(hbSysFlushMem(&t.sysMem[0],HB_SYS_MEM_CACHE_INVALIDATE),"invalidate");
    return ms;
  }
  cv::Mat InputAsMat(int plane,int rows,int cols,int type) {
    auto& p=inputs_[0].properties;
    int h=p.validShape.dimensionSize[2],w=p.validShape.dimensionSize[3];
    auto* base=static_cast<uint8_t*>(inputs_[0].sysMem[0].virAddr);
    return cv::Mat(rows,cols,type,base+(plane==1?h*w:0));
  }
  void* InputMem(int) {return inputs_[0].sysMem[0].virAddr;}
  void* OutputMem(int i) {return outputs_[i].sysMem[0].virAddr;}
  const hbDNNTensorProperties& InputProps(int i) const {return inputs_[i].properties;}
  const hbDNNTensorProperties& OutputProps(int i) const {return outputs_[i].properties;}
  int input_count() const {return inputs_.size();}
  int output_count() const {return outputs_.size();}
...
static void FindNv12Inputs(DnnModel* m, int* y_idx, int* uv_idx, int* size) {
  *y_idx = 0; *uv_idx = 1;
  const auto& p=m->InputProps(0);
  if(p.validShape.numDimensions !=4 || p.validShape.dimensionSize[2]!=p.validShape.dimensionSize[3])
    throw std::runtime_error("expected square NCHW NV12 input");
  *size=p.validShape.dimensionSize[2];
}

class DetectModel {
 public:
  DetectModel(hbPackedDNNHandle_t packed, const std::string& name,
    ...
    if(num_outputs_!=6)throw std::runtime_error("YOLO26 detect requires 6 outputs");
    for(int i=0;i<6;++i) {
      const auto& p=model_.OutputProps(i);
      if(p.validShape.numDimensions!=4 || p.validShape.dimensionSize[3]!=(i%2?4:80))
        throw std::runtime_error("unexpected YOLO26 detection output shape");
    }
...
class DepthModel {
 public:
  DepthModel(hbPackedDNNHandle_t packed, const std::string& name,
    ...
    const auto& out=model_.OutputProps(0).validShape;
    if(model_.output_count()!=1 || out.numDimensions!=4 || out.dimensionSize[3]!=1 || out.dimensionSize[1]!=out.dimensionSize[2])
      throw std::runtime_error("expected square NHWC depth output");
    out_size_=out.dimensionSize[1];
  }
  ...
  double Preprocess(const cv::Mat& bgr) {
    ...
    if (profile_ == "nv12") {
      ...
      if (bgr.cols != rw || bgr.rows != rh)
        FastResize(bgr, resized, cv::Size(rw, rh), 0, 0, cv::INTER_LINEAR);
      else
        resized = bgr;
      ...
      const int S = size_;
      cv::Mat rgb;
      FastResize(bgr, rgb, cv::Size(S, S), 0, 0, cv::INTER_LINEAR);
  ...
  double Postprocess(const cv::Mat& bgr, cv::Mat& color_out) {
    ...
    if (profile_ == "nv12") {
      ...
      cv::Mat big;
      FastResize(color_small, big, cv::Size(size_, size_), 0, 0,
                 cv::INTER_LINEAR);
      cv::Mat cropped = big(crop_);
      FastResize(cropped, color_out, cv::Size(bgr.cols, bgr.rows), 0, 0,
                 cv::INTER_LINEAR);
    } else {
      // lite: input was stretched, no letterbox geometry
      FastResize(color_small, color_out, cv::Size(bgr.cols, bgr.rows), 0, 0,
                 cv::INTER_LINEAR);
    }
    return ms_since(t0);
  }

  void DumpLogDepth(const std::string& path) {
    std::ofstream f(path,std::ios::binary);
    f.write(static_cast<const char*>(model_.OutputMem(0)),out_size_*out_size_*sizeof(float));
    if(!f)throw std::runtime_error("cannot save depth output");
  }
  int input_size() const { return size_; }
...
class FrameHub {
  ...
  void publish(const cv::Mat& f) {
    {
      ...
      frame_ = f; captured_=NOW();
      ++seq_;
    }
    cv_.notify_all();
  }
  // wait for a frame newer than last_seq
  bool latest(uint64_t& last_seq, cv::Mat& out, double timeout_s, fclock::time_point* captured=nullptr) {
    ...
    last_seq = seq_;
    out = frame_;
    if(captured)*captured=captured_;
    return true;
  }
  uint64_t seq() {
    std::lock_guard<std::mutex> lk(mu_);
 ...
 private:
  ...
  cv::Mat frame_;
  fclock::time_point captured_{};
  uint64_t seq_ = 0;
};

struct DetResult {
  uint64_t source_seq = 0;
  fclock::time_point captured{};
  std::vector<Det> dets;
...
struct SharedImage {
  ...
  uint64_t version = 0, source_seq = 0;
  void set(const std::vector<uchar>& j, uint64_t seq) {
    {
      std::lock_guard<std::mutex> lk(mu);
      jpeg = j; source_seq=seq;
      ++version;
    }
    cv.notify_all();
  }
...
struct CompositeSlot {
  ...
  cv::Mat img;
  uint64_t version = 0, source_seq = 0;
  void set(const cv::Mat& m, uint64_t seq) {
    {
      std::lock_guard<std::mutex> lk(mu);
      source_seq=seq;
      img = m; // publish immutable storage; never overwrite a reader's pixels
      ++version;
    }
    cv.notify_all();
  }
...
static void DrawYoloBox(cv::Mat& im, const Det& d,
  ...
  const int lw = 3;
  const double sf = std::min(0.70, im.cols / 1100.0);
...
static void DrawHud(cv::Mat& pane, const std::string& left_txt,
  ...
  int y=18;
  for(const std::string& text : {left_txt,right_txt}) {
    if(text.empty()) continue;
    int baseline=0;
    double sf=0.45;
    auto ts=cv::getTextSize(text,cv::FONT_HERSHEY_SIMPLEX,sf,1,&baseline);
    if(ts.width>pane.cols-24) sf*=double(pane.cols-24)/ts.width;
    cv::rectangle(pane,{6,y-14,pane.cols-12,21},cv::Scalar(20,22,26),-1);
    cv::rectangle(pane,{6,y-14,3,21},accent,-1);
    cv::putText(pane,text,{14,y+1},cv::FONT_HERSHEY_SIMPLEX,sf,cv::Scalar(240,240,240),1,cv::LINE_AA);
    y+=22;
  }
}
...
static void DrawDepthScale(cv::Mat& pane) {
  ...
  const cv::Rect r(pane.cols - bar_w - 12, (pane.rows - bar_h) / 2, bar_w,
                   bar_h);
  cv::Mat bar;
  FastResize(lut, bar, r.size(), 0, 0, cv::INTER_LINEAR);
  bar.copyTo(pane(r));
...
static cv::Mat DrawSidePanel(const cv::Size& canvas, const PanelMetrics& m,
  ...
  PnlText(im, "YOLO26 DETECT + DEPTH", {ml, (int)(26 * vs)}, 0.62 * fs,
          kPnlText, 2);
  PnlText(im, "RDK X5 - hbDNN C++ dual model", {ml, (int)(46 * vs)},
          0.36 * fs, kPnlMuted);
  ...
  model_card("YOLO26 DETECT", kPnlBlue, m.det_avg, m.det_p95, m.det_min,
             m.det_max, m.det_fps, m.det_hist, "640x640 NV12");
  model_card("YOLO26 DEPTH", kPnlGreen, m.dep_avg, m.dep_p95, m.dep_min,
             m.dep_max, m.dep_fps, m.dep_hist,
             depth_meters > 0 ? "768 NV12 - METERS" : "768 NV12 - RAW");
  ...
  // turbo legend
  {
    ...
    cv::Mat roi = im(cv::Rect(ml + (int)(38 * fs), y + fh / 2 - (int)(6 * vs),
                              ramp.cols, (int)(12 * vs)));
    FastResize(turbo, roi, roi.size(), 0, 0, cv::INTER_LINEAR);
...
static void DrawQrColumn(cv::Mat& canvas, const cv::Point& org, int total_w,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (!qr[i].empty()) {
      ...
      cv::Mat rs;
      FastResize(qr[i], rs, inner.size(), 0, 0,
...
static std::string LoadIndexHtml() {
  ...
  return
      ...
      "</style></head><body><h3>RDK X5 - YOLO26 Detect + Depth</h3>"
...
class HttpServer {
 public:
  ...
  void stop() {
    ...
    clients_cv_.wait(lk,[&]{return clients_==0;});
  }
 ...
 private:
  void acceptLoop() {
    while (!g_stop.load()) {
      ...
      {std::lock_guard<std::mutex> lk(clients_mu_); ++clients_;}
      std::thread([this,c] {
        try {serve(c);} catch(...) {close(c);}
        {std::lock_guard<std::mutex> lk(clients_mu_); --clients_;}
  ...
  void serve(int c) {
    ...
    while (req.find("\r\n\r\n") == std::string::npos) {
      ...
      if(req.size()>16384) {close(c); return;}
    }
    ...
    if (path == "/stream") {
      ...
      static const std::string qr_dir = "web/";
  ...
  std::mutex clients_mu_; std::condition_variable clients_cv_; int clients_=0;
...
struct Args {
  std::string det_model, dep_model, dep_variant = "n";
  int grid_cols = 4, grid_rows = 3;
  bool no_grid = false;
  double depth_meters = 0;  // 0 = raw normalized; >0 = meters calibration
  std::string source = "0";
  int cam_w = 640, cam_h = 480, cam_fps = 30;   // 1080p@30 default (PIXY-capable)
  int port = 8080;
...
static Args ParseArgs(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (k == "--verify-dir") a.verify_dir = next();
    else if (k == "--hdmi") a.hdmi = true;
    else if (k == "--track") a.tracking = true;
    else {
      std::fprintf(stderr, "unknown arg: %s\n", k.c_str());
      std::exit(2);
    }
  }
  if(a.cam_w<160||a.cam_h<90||a.cam_w>1920||a.cam_h>1080||a.cam_fps<1||a.cam_fps>60||
     a.cam_w%2||a.cam_h%2||a.port<1||a.port>65535||a.score<=0||a.score>=1||a.nms<=0||a.nms>=1||
     a.bpu_core!=0||a.prio_det<0||a.prio_det>255||a.prio_dep<0||a.prio_dep>255||
     a.grid_cols<0||a.grid_cols>16||a.grid_rows<0||a.grid_rows>12||a.warmup<0||a.max_seconds<0)
    throw std::runtime_error("invalid size/fps/threshold/priority/grid; X5 only supports core 0");
  return a;
}
...
int main(int argc, char** argv) {
  ...
  cv::setNumThreads(1);
  if(args.det_model.empty()) args.det_model="models/yolo26n_detect_bayese_640x640_nv12.bin";
  if(args.dep_model.empty()) args.dep_model="models/yolo26n_depth_bayese_768x768_nv12.bin";
  if(args.depth_meters != 0) {
    std::fprintf(stderr,"relative normalized depth cannot be converted to meters by one scale\n"); return 2;
  }
  std::string det_label=basename_of(args.det_model).substr(0,7);
  std::string dep_label=basename_of(args.dep_model).substr(0,7);
  std::transform(det_label.begin(),det_label.end(),det_label.begin(),::toupper);
  std::transform(dep_label.begin(),dep_label.end(),dep_label.begin(),::toupper);
  ...
  std::printf("[dnn  ] X5 hbDNN version=%s\n",hbDNNGetVersion());
  ...
  if (!args.image_mode.empty()) {
    ...
    double post1 = det.Postprocess(
        img.cols, img.rows, (args.tracking ? std::min(args.score, args.track_low_thresh) : args.score),
        args.score, args.nms, dr.dets, source_dets, dr.cls_hist);
    if(args.tracking) dr.tracks = tracker.Update(tracking::SelectPersonDetections(
    ...
    if(!args.tracking) dr.object_count = dr.dets.size();
    ...
    if(!args.verify_dir.empty()) {
      std::filesystem::create_directories(args.verify_dir);
      dep.DumpLogDepth(args.verify_dir+"/log_depth.f32");
      std::ofstream f(args.verify_dir+"/detections.json"); f<<"[";
      bool first=true;
      for(const auto& d:dr.dets) {
        if(!first)f<<",";first=false;
        f<<"["<<d.x1<<","<<d.y1<<","<<d.x2<<","<<d.y2<<","<<d.score<<","<<d.cls<<"]";
      }
      f<<"]";
    }
    // composite
    ...
    for (const auto& d : dr.dets) {
      if (!args.tracking || tracking::ShouldRenderRawDetection(d.cls)) {
        DrawYoloBox(top, d, labels);
      }
    }
    for (const auto& track : dr.tracks) ui::DrawTrackBox(top, track);
    DrawHud(top, det_label + " DETECT 640x640 NV12 | BPU " +
    ...
    DrawHud(depth_color, dep_label + " DEPTH 768x768 NV12 | BPU " +
                             std::to_string(bpu2).substr(0, 5) +
                             " ms | Turbo",
            "Relative depth 0=near / 1=far (not meters)", kUltra[6]);
  ...
  if (is_camera) {
    ...
    for (int attempt = 1; attempt <= 10; ++attempt) {
      if(is_cam_idx)cap.open(std::stoi(args.source),cv::CAP_V4L2);
      else cap.open(args.source,cv::CAP_V4L2);
  ...
  std::atomic<uint64_t> composed_count{0}, encoded_count{0};
  ...
  std::thread det_worker([&] {
    cv::Mat frame;
    uint64_t seq = 0; fclock::time_point captured{};
    // warmup
    for (int i = 0; i < args.warmup; ++i) {
    ...
    while (!g_stop.load()) {
      ...
      DetResult r; r.source_seq=seq; r.captured=captured;
      r.src_w = frame.cols;
      r.src_h = frame.rows;
      ...
      r.post_ms = det.Postprocess(
          ...
          (args.tracking ? std::min(args.score, args.track_low_thresh) : args.score), args.score, args.nms,
          r.dets, source_dets, r.cls_hist);
      auto person_inputs = tracking::SelectPersonDetections(
          source_dets, args.track_low_thresh);
      auto track_t0 = NOW();
      try {
        if(args.tracking) r.tracks = tracker.Update(person_inputs);
      ...
      if(args.tracking) r.cls_hist[0] = r.tracker_stats.active;
      r.object_count = tracking::CountDisplayedObjects(
          r.tracker_stats.active, source_dets, args.score);
      if(!args.tracking) r.object_count = r.dets.size();
  ...
  std::thread dep_worker([&] {
    ...
    while (!g_stop.load()) {
      if (!hub.latest(seq, frame, 1.0,&captured)) continue;
      DepResult r; r.source_seq=seq; r.captured=captured;
  ...
  std::thread composer([&] {
    ...
    int fps_counter = 0;
    auto fps_t0 = NOW();
    cv::Mat frame, cached_bottom;
    uint64_t cached_depth_seq=0;
    while (!g_stop.load()) {
      if(!hub.latest(last_frame_seq,frame,0.1)) continue;
      ...
      DrawHud(top,
              ...
              kUltra[2]);
      if(cached_bottom.empty() || cached_depth_seq != dpr.source_seq) {
        cached_bottom=dpr.color.empty()?cv::Mat(frame.size(),CV_8UC3,cv::Scalar(0,0,0)):dpr.color.clone();
        DrawDepthScale(cached_bottom);
        DrawDepthGrid(cached_bottom,dpr.grid,dpr.grid_cols,dpr.grid_rows,args.depth_meters);
        DrawHud(cached_bottom,dep_label + " DEPTH 768 NV12 | BPU "+fmt(dpr.bpu_ms)+" ms",
                "Relative depth 0=near / 1=far (not meters)",kUltra[6]);
        cached_depth_seq=dpr.source_seq;
      }
      ...
      const int pane_h = frame.rows;                // compressed pane height
      ...
      auto letterbox = [&](const cv::Mat& src, cv::Mat& out) {
        ...
        cv::Mat resized;
        FastResize(src, resized, cv::Size(w,h));
      ...
      comp_slot.set(composite,last_frame_seq);
      if (!args.save.empty()) {
        if (!writer.isOpened()) {
  ...
  // ---- HDMI KMS output -------------------------------------------------------
  KmsDisplay kms;
  std::thread hdmi_thread, hist_feeder;
  ...
  if (args.hdmi) {
    hdmi_thread = std::thread([&] {
      ...
      auto build_caches = [&]() {
        ...
        pm.bpu_freq_mhz = atoll(read_first_line("/sys/class/devfreq/3a000000.bpu/cur_freq").c_str())/1000000;
      ...
      while (!g_stop.load()) {
        ...
        if (!panel_cache.empty() && !gauge_cache.empty()) {
          ...
          cv::Mat rs;
          FastResize(composite, rs, vroi.size(), 0, 0, cv::INTER_AREA);
          rs.copyTo(vroi);
          // metric panel right
          ...
          cv::Mat rs;
          FastResize(composite, rs, cv::Size(W, H), 0, 0, cv::INTER_AREA);
          rs.copyTo(canvas);
        }
        kms.Present(canvas);
  ...
  auto build_stats = [&]() -> std::string {
    ...
    double det_span = std::max(now_s, 1.0);
    ...
    long long freq_mhz = atoll(
        read_first_line("/sys/class/devfreq/3a000000.bpu/cur_freq").c_str()) /
                         1000000;
    std::ostringstream o;
    o << std::fixed << std::setprecision(1)
      << "{\"uptime_s\":" << now_s << ",\"display_fps\":" << display_fps.load()
      << ",\"composed_frames\":" << composed_count.load()
      << ",\"encoded_frames\":" << encoded_count.load()
    ...
    o << "\"detect\":{\"frames\":" << det_count.load() << ",\"fps\":" << det_fps
      ...
      << ",\"source_seq\":" << tr.source_seq
      << ",\"age_ms\":" << (tr.source_seq?ms_since(tr.captured):0)
    ...
    o << "]},\"tracking\":{"
      << "\"enabled\":" << (args.tracking ? "true" : "false") << ","
    ...
    o << "\"depth\":{\"frames\":" << dep_count.load()
      ...
      << ",\"source_seq\":" << store.dep().source_seq
      << ",\"age_ms\":" << (store.dep().source_seq?ms_since(store.dep().captured):0)
      << ",\"profile\":\"" << dep.profile() << "\""
    ...
    o << "\"models\":{\"detect\":{\"file\":\"" << basename_of(args.det_model)
      << "\",\"input\":\"640x640 NV12\",\"latency_bench_ms\":null},"
      << "\"depth\":{\"file\":\"" << basename_of(args.dep_model)
      << "\",\"input\":\"" << dep.input_size() << "x" << dep.input_size() << " "
      << (dep.profile() == "lite" ? "F32 lite" : "NV12")
      << "\",\"latency_bench_ms\":null}},";
    o << "\"system\":{\"soc\":\"" << "RDK X5 (Bayes-e)" << "\",\"dnn\":\""
      << hbDNNGetVersion() << "\",\"ucp\":\"" << "not used on X5" << "\"}}";
    return o.str();
  };
  ...
  if (hist_feeder.joinable()) hist_feeder.join();
  http.stop();
  hbDNNRelease(packed);
