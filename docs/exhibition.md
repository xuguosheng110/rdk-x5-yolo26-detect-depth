...
停止自启动时，先停止浏览器服务，将 `rdk-x5-vision.desktop` 移出 autostart 目录，再执行 `systemctl disable --now rdk-x5-vision`。要恢复普通桌面策略，可从备份恢复同名文件，移除本项目新增的 LightDM 覆盖项，并在 Xfce 电源设置中重新开启需要的节能选项。
