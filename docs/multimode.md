...
高清显示将原始左目 1280×1088 JPEG、640×480 深度伪彩和该深度帧的采样数值打包到 `/stereo.frame`，一次传输保持图文一致。浏览器独立绘制文字，原图上不再铺满距离网格；SPI 减少数字数量。左右配图按时间戳选择最近图像，差值超过 500ms 则不发布。兼容 `/frame.jpg` 仍提供双幅 JPEG，但电脑交互页面使用高清帧包。
