...
| 双目深度 | 校正后的左右目 → 官方 DStereo V2.4 int16 320×256 | 使用模组标定输出深度估计；需用已知距离验证 |
...
高清显示将原始左目 1280×1088 JPEG、320×256 深度伪彩和该深度帧的采样数值打包到 `/stereo.frame`，一次传输保持图文一致。浏览器在原图与深度图上独立绘制对应的距离框，电脑端保留 4×3 采样网格；SPI 同样显示每侧全部 12 个距离数值，以紧凑字号和居中标签适配小屏。右侧深度图已按用户要求恢复此前的平滑缩放显示。左右配图按时间戳选择最近图像，差值超过 500ms 则不发布。兼容 `/frame.jpg` 仍提供双幅 JPEG，但电脑交互页面使用高清帧包。
...
| 文件 / 服务 | 作用 |
|---|---|
| `config/rdk-x5-camera.service` | 常驻双目采集，15fps 请求，主路 1280×1088、子路 320×256、GDC 校正、LPWM 同步、实时戳 |
...
10. **StereoNet 内参重复缩放**：当前包接收大图的 CameraInfo 后先按模型尺寸缩放内参，图像 resize 路径又缩放一次。启用 320×256 子码流并同时指定对应左右 CameraInfo，绕开这条路径。不要只改变图像话题而沿用主码流内参。
